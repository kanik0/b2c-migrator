...
mod tests {
    ...
    fn test_dblogger_insert_line_short_fallback() {
        ...
        let row: (String, String, String) = stmt
            .query_row([], |r| Ok((r.get(0).unwrap(), r.get(1).unwrap(), r.get(2).unwrap())))
            .unwrap();
    ...
    fn test_dblogger_insert_line_username_without_quotes() {
        ...
        let conn = conn_arc.lock().unwrap();
        let mut stmt = conn.prepare(&format!("SELECT username FROM '{}'", table_name)).unwrap();
    ...
    fn test_dblogger_write_and_flush() {
        ...
        logger.write_all(b"2024-01-02 11:00:00 [INFO] [user1] First part.").unwrap();
        logger.write_all(b" Still user1.\n").unwrap();
        logger.write_all(b"2024-01-02 11:01:00 [WARN] [user2] Second line fully.\n").unwrap();

        // At this point, two lines should be in the DB
        let conn_check1 = conn_arc.lock().unwrap();
        let count1: i64 = conn_check1.query_row(&format!("SELECT COUNT(*) FROM '{}'", table_name), [], |r| r.get(0)).unwrap();
        assert_eq!(count1, 2, "Should have 2 rows after two full lines with newlines");
        drop(conn_check1);


        // Write a partial line, then flush
        logger.write_all(b"2024-01-02 11:02:00 [ERROR] [user3] Partial flush").unwrap();
        logger.flush().unwrap();

        let conn_check2 = conn_arc.lock().unwrap();
        let count2: i64 = conn_check2.query_row(&format!("SELECT COUNT(*) FROM '{}'", table_name), [], |r| r.get(0)).unwrap();
        assert_eq!(count2, 3, "Should have 3 rows after flush");

        let last_message: String = {
            let mut stmt = conn_check2.prepare(&format!("SELECT message FROM '{}' ORDER BY id DESC LIMIT 1", table_name)).unwrap();
        ...
        let count3: i64 = conn_check3.query_row(&format!("SELECT COUNT(*) FROM '{}'", table_name), [], |r| r.get(0)).unwrap();
        assert_eq!(count3, 3, "Count should remain 3 after flushing empty buffer");
    }
    ...
    fn test_dblogger_write_multiple_lines_in_one_buffer() {
        ...
        let conn = conn_arc.lock().unwrap();
        let count: i64 = conn.query_row(&format!("SELECT COUNT(*) FROM '{}'", table_name), [], |r| r.get(0)).unwrap();
        assert_eq!(count, 2);

        let mut stmt = conn.prepare(&format!("SELECT message FROM '{}' ORDER BY id", table_name)).unwrap();
        let messages: Vec<String> = stmt.query_map([], |r| r.get(0)).unwrap().map(|res| res.unwrap()).collect();
    ...
    fn test_dblogger_write_empty_string() {
        ...
        assert_eq!(count, 0, "No rows should be inserted for empty write and flush");
    }
